...
CORS(app, origins=[
    "http://localhost:3000",
    "https://lidar-frontend-zeta.vercel.app/"  # ← your Vercel URL
])
...
def process_contour():
    ...
    try:
        ...
        # Re-hydrate InputDataProcessor from JSON payload
        processor = InputDataProcessor()
...
def compute_metrics():
    ...
    try:
        ...
        if len(contour_a) < 2 or len(contour_b) < 2:
            return (
                jsonify({"error": "Both contours must have at least 2 points"}),
                400,
            )
